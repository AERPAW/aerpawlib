...
For multi-vehicle missions: start `aerpawlib-run-proxy`, pass `--zmq-identifier` and `--zmq-proxy-server` on each runner, and call `await self.wait_for_peers([...])` before the first remote command.
...
# set_velocity is not supported on the AERPAW testbed
