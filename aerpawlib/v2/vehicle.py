...
from .types import Coordinate, VectorNED, Attitude, DroneState, FlightMode, LandedState, Waypoint
from .exceptions import (
    ConnectionError as AerpawConnectionError,
    ConnectionTimeoutError, ReconnectionError, CommandError, ArmError, DisarmError,
    TakeoffError, TakeoffTimeoutError, LandingError, LandingTimeoutError,
    NavigationError, GotoTimeoutError, ModeChangeError,
    OffboardError as AerpawOffboardError,
    TimeoutError as AerpawTimeoutError, AbortError, NotArmableError, CommandCancelledError,
    PreflightCheckError, ParameterValidationError,
)
from .safety import (
...
class CommandHandle:
    ...
    def is_complete(self) -> bool:
        return self._status in (CommandStatus.COMPLETED, CommandStatus.FAILED, CommandStatus.CANCELLED, CommandStatus.TIMED_OUT)
    ...
    def result(self) -> CommandResult:
        ...
        return CommandResult(self._status, self._command, self._start_time, self._end_time or time.time(), self._error, self.progress)
        ...
        try:
            while not self._cancelled:
                if self._completion_condition():
                    self._status, self._end_time = CommandStatus.COMPLETED, time.time()
                    self._completion_event.set()
                    return
                if self._abort_checker and self._abort_checker():
                    self._status, self._error = CommandStatus.CANCELLED, AbortError(message="Vehicle abort triggered")
                    self._end_time = time.time()
                    self._completion_event.set()
                    return
                if self._timeout and self.elapsed_time > self._timeout:
                    self._status = CommandStatus.TIMED_OUT
                    self._error = AerpawTimeoutError(message=f"{self._command} timed out after {self._timeout}s", timeout=self._timeout)
                    self._end_time = time.time()
                    self._completion_event.set()
                    return
            ...
            self._status, self._error, self._end_time = CommandStatus.FAILED, e, time.time()
            self._completion_event.set()
    ...
    async def wait(self, timeout: Optional[float] = None) -> "CommandHandle":
        ...
        try:
            if wait_timeout:
                await asyncio.wait_for(self._completion_event.wait(), timeout=wait_timeout)
            else:
                await self._completion_event.wait()
        except asyncio.TimeoutError:
            self._status = CommandStatus.TIMED_OUT
            self._error = AerpawTimeoutError(message=f"Wait for {self._command} timed out", timeout=wait_timeout)
            self._end_time = time.time()
        return self._raise_if_error()
...
class StateContainer:
    ...
    def snapshot(self) -> DroneState:
        return DroneState(
            heading=self.heading, velocity=self.velocity, attitude=self.attitude,
            altitude=self.altitude, relative_altitude=self.relative_altitude,
            latitude=self.latitude, longitude=self.longitude, groundspeed=self.groundspeed,
            airspeed=self.airspeed, climb_rate=self.climb_rate,
            flight_mode=self.flight_mode, landed_state=self.landed_state,
        )
...
class GPSContainer:
    ...
    def quality(self) -> str:
        return {0: "No GPS", 1: "No Fix", 2: "2D Fix", 3: "3D Fix", 4: "DGPS", 5: "RTK Float", 6: "RTK Fixed"}.get(self.fix_type, "Unknown")
...
class InfoContainer:
    ...
    def time_since_arm_ms(self) -> int:
        return int((time.time() - self.arm_time) * 1000) if self.arm_time else 0

    @property
    def time_since_takeoff_ms(self) -> int:
        return int((time.time() - self.takeoff_time) * 1000) if self.takeoff_time else 0
...
class Vehicle:
    ...
    def __init__(self, connection: str = "udp://:14540"):
        ...
        self._callbacks: Dict[VehicleEvent, List[CallbackType]] = {event: [] for event in VehicleEvent}
            ...
            try:
                ...
                async for conn_state in self._system.core.connection_state():
                    ...
                    if time.time() - start_time > timeout:
                        raise ConnectionTimeoutError(timeout=timeout, address=self._connection_string, attempt=attempt, max_attempts=retry_count)
                    await asyncio.sleep(_POLLING_DELAY)
                ...
                last_error = AerpawConnectionError(message=f"Connection failed: {e}", address=self._connection_string, attempt=attempt, max_attempts=retry_count)
    ...
    async def _attempt_reconnect(self) -> bool:
        ...
        try:
            ...
            if self._connected:
                await self._start_telemetry()
                await self._trigger_callbacks(VehicleEvent.RECONNECT)
                return True
        except Exception as e:
    ...
    async def _handle_position(self, p):
        self.state.latitude, self.state.longitude = p.latitude_deg, p.longitude_deg
        self.state.altitude, self.state.relative_altitude = p.absolute_altitude_m, p.relative_altitude_m

    async def _handle_attitude(self, a):
        self.state.attitude = Attitude(roll=math.radians(a.roll_deg), pitch=math.radians(a.pitch_deg), yaw=math.radians(a.yaw_deg))
    ...
    async def _handle_velocity(self, v):
        ...
        self.state.groundspeed, self.state.climb_rate = math.hypot(v.north_m_s, v.east_m_s), -v.down_m_s

    async def _handle_gps(self, g):
        self.gps.satellites, self.gps.fix_type = g.num_satellites, g.fix_type.value

    async def _handle_battery(self, b):
        self.battery.voltage, self.battery.charge = b.voltage_v, b.remaining_percent / 100.0
    ...
    async def _handle_flight_mode(self, mode):
        ...
        if old != self.state.flight_mode:
            await self._trigger_callbacks(VehicleEvent.MODE_CHANGE, old, self.state.flight_mode)
    ...
    async def _handle_health(self, h):
        self._is_armable = h.is_global_position_ok and h.is_home_position_ok and h.is_armable

    async def _handle_home(self, h):
        self._home = Coordinate(h.latitude_deg, h.longitude_deg, h.relative_altitude_m)
    ...
    def _capture_telemetry_snapshot(self) -> Dict[str, Any]:
        return {
            "timestamp": time.time(), "latitude": self.state.latitude, "longitude": self.state.longitude,
            "altitude": self.state.relative_altitude, "heading": self.state.heading, "groundspeed": self.state.groundspeed,
            "velocity_north": self.state.velocity.north, "velocity_east": self.state.velocity.east,
            "velocity_down": self.state.velocity.down, "roll": self.state.attitude.roll, "pitch": self.state.attitude.pitch,
            "yaw": self.state.attitude.yaw, "battery_voltage": self.battery.voltage, "battery_percent": self.battery.charge * 100,
            "gps_satellites": self.gps.satellites, "gps_fix_type": self.gps.fix_type, "armed": self._armed, "flight_mode": self.state.flight_mode.name,
        }
    ...
    def save_flight_log(self, path: str, format: str = "json") -> None:
        ...
        if format == "json":
            with open(path, "w") as f:
                json.dump({"metadata": {"recorded_at": time.strftime("%Y-%m-%d %H:%M:%S"), "data_points": len(self._flight_log), "interval": self._recording_interval}, "telemetry": self._flight_log}, f, indent=2)
        elif format == "csv":
    ...
    async def _trigger_callbacks(self, event: VehicleEvent, *args, **kwargs) -> None:
    ...
    async def wait_for_gps_fix(self, min_satellites: int = 6, timeout: float = 60.0) -> bool:
        await self._await_condition(lambda: self.gps.has_fix and self.gps.satellites >= min_satellites, timeout=timeout, message=f"GPS fix timeout after {timeout}s")
        return True

    async def wait_for_armable(self, timeout: float = 60.0) -> bool:
        await self._await_condition(lambda: self._is_armable, timeout=timeout, message=f"Armable timeout after {timeout}s")
        return True
    ...
    async def get_param(self, name: str) -> float:
        try:
            ...
            raise CommandError(message=f"Failed to get parameter '{name}': {e}", command="get_param", reason=str(e))
    ...
    async def set_param(self, name: str, value: float) -> bool:
        try:
            ...
            raise CommandError(message=f"Failed to set parameter '{name}': {e}", command="set_param", reason=str(e))
    ...
    def connection_healthy(self) -> bool:
        return self._connected and (time.time() - self._last_heartbeat) < _HEARTBEAT_TIMEOUT

    @property
    def seconds_since_heartbeat(self) -> float:
        return float('inf') if self._last_heartbeat == 0 else time.time() - self._last_heartbeat
    ...
    async def arm(self, force: bool = False) -> bool:
        ...
        if not force and not self._is_armable:
            raise NotArmableError(message="Vehicle is not armable. Check GPS and other pre-flight conditions.")
        try:
    ...
    async def _await_condition(self, condition: Callable[[], bool], timeout: Optional[float] = None, message: str = "Condition not met"):
        start = time.time()
        while not condition():
...
class Drone(Vehicle):
    ...
    async def takeoff(self, altitude: float = 5.0, wait: bool = True, timeout: float = 60.0) -> Optional[CommandHandle]:
        ...
        if self._safety_checker:
            await validate_takeoff_with_checker(self._safety_checker, altitude, self.state.latitude, self.state.longitude, raise_on_fail=True)

        try:
            await self._system.action.set_takeoff_altitude(altitude)
            await self._system.action.takeoff()
        except ActionError as e:
            raise TakeoffError(message=f"Takeoff failed: {e}", target_altitude=altitude, reason=str(e))
        ...
        if wait:
            try:
                ...
                raise TakeoffTimeoutError(target_altitude=altitude, current_altitude=self.state.relative_altitude, timeout=timeout)
            return None
        return handle
    ...
    async def land(self, wait: bool = True, timeout: float = 120.0) -> Optional[CommandHandle]:
        ...
        try:
            ...
            raise LandingError(message=f"Landing failed: {e}", current_altitude=self.state.relative_altitude, reason=str(e))

        handle = CommandHandle(
            command="land", completion_condition=lambda: not self._armed, cancel_action=None,
            timeout=timeout, abort_checker=None,
            progress_getter=lambda: {"current_altitude": self.state.relative_altitude, "landed_state": self.state.landed_state.name, "armed": self._armed},
        )
        handle._start()
        ...
        if wait:
            try:
                ...
                raise LandingTimeoutError(current_altitude=self.state.relative_altitude, timeout=timeout)
            return None
        return handle
    ...
    async def rtl(self, wait: bool = True, timeout: float = 300.0) -> Optional[CommandHandle]:
        ...
        def progress():
            r = {"current_altitude": self.state.relative_altitude, "landed_state": self.state.landed_state.name, "armed": self._armed}
            if self._home:
                r["distance_to_home"] = self.position.distance_to(self._home)
            return r

        handle = CommandHandle(command="rtl", completion_condition=lambda: not self._armed, cancel_action=None, timeout=timeout, abort_checker=None, progress_getter=progress)
        handle._start()
        if wait:
            await handle.wait()
            return None
    ...
    async def goto(self, latitude: Optional[float] = None, longitude: Optional[float] = None, altitude: Optional[float] = None, coordinates: Optional[Coordinate] = None, tolerance: float = 2.0, speed: Optional[float] = None, heading: Optional[float] = None, timeout: float = 300.0, wait: bool = True) -> Optional[CommandHandle]:
        target = coordinates if coordinates else Coordinate(latitude, longitude, altitude or self.state.relative_altitude) if latitude and longitude else None
        if not target:
            raise ValueError("Must provide either coordinates or latitude/longitude")

        if self.safety_limits.enable_parameter_validation:
            for name, check, val in [("target", validate_coordinate(target, "target"), target), ("altitude", validate_altitude(target.altitude, "altitude"), target.altitude), ("tolerance", validate_tolerance(tolerance, "tolerance"), tolerance), ("timeout", validate_timeout(timeout, "timeout"), timeout)]:
                if not check:
                    raise ParameterValidationError(message=check.message, parameter=name, value=val)
            if speed:
                check = validate_speed(speed, self.safety_limits, "speed")
                if not check:
                    speed = clamp_speed(speed, self.safety_limits) if self.safety_limits.auto_clamp_values else None
                    if speed is None:
                        raise ParameterValidationError(message=check.message, parameter="speed", value=speed)

        if self._safety_checker:
            await validate_waypoint_with_checker(self._safety_checker, self.position, target, raise_on_fail=True)
            if speed:
                await validate_speed_with_checker(self._safety_checker, speed, raise_on_fail=True)

        if speed:
            await self._system.action.set_maximum_speed(speed)
        self._current_heading = heading or self._current_heading or self.position.bearing_to(target)

        try:
            await self._system.action.goto_location(target.latitude, target.longitude, target.altitude + (self._home.altitude if self._home else 0), self._current_heading or 0)
        except ActionError as e:
            raise NavigationError(message=f"Goto failed: {e}", target=target, current_position=self.position, reason=str(e))

        handle = CommandHandle(command="goto", completion_condition=lambda: self.position.distance_to(target) <= tolerance, cancel_action=self.hold, timeout=timeout, abort_checker=lambda: self._aborted, progress_getter=lambda: {"distance": self.position.distance_to(target), "target": target, "tolerance": tolerance})
        handle._start()
        ...
        if wait:
            try:
                ...
                raise GotoTimeoutError(target=target, distance_remaining=self.position.distance_to(target), timeout=timeout)
            return None
        return handle
    ...
    async def set_heading(self, degrees: float, blocking: bool = True, timeout: float = 30.0) -> Optional[CommandHandle]:
        ...
        try:
            await self._system.offboard.set_position_ned(PositionNedYaw(0, 0, -self.state.relative_altitude, self._current_heading))
            await self._system.offboard.start()
        except (OffboardError, ActionError) as e:
            raise ModeChangeError(message=f"Heading change failed: {e}", target_mode="offboard", reason=str(e))
        ...
        handle = CommandHandle(command="set_heading", completion_condition=lambda: diff() < 5, cancel_action=stop, timeout=timeout, abort_checker=lambda: self._aborted, progress_getter=lambda: {"current_heading": self.state.heading, "target_heading": target_heading, "heading_diff": diff()})
        handle._start()
    ...
    async def move_in_current_direction(self, distance: float, speed: float = 5.0):
        await self.move_in_direction(distance, self._current_heading or self.state.heading, speed)

    async def move_in_direction(self, distance: float, degrees: float, speed: float = 5.0):
        rad = math.radians(degrees)
        await self.goto(coordinates=self.position + VectorNED(distance * math.cos(rad), distance * math.sin(rad), 0), speed=speed)

    async def move_towards(self, target: Coordinate, distance: float, speed: float = 5.0):
        await self.move_in_direction(distance, self.position.bearing_to(target), speed)

    async def set_velocity(self, velocity: VectorNED, heading: Optional[float] = None, duration: Optional[float] = None, wait: bool = True) -> Optional[CommandHandle]:
        if heading:
            self._current_heading = heading
        try:
            await self._system.offboard.set_velocity_ned(VelocityNedYaw(velocity.north, velocity.east, velocity.down, self._current_heading or self.state.heading))
            try:
                await self._system.offboard.start()
            ...
            raise AerpawOffboardError(message=f"Set velocity failed: {e}", reason=str(e))
    ...
    async def set_altitude(self, altitude: float, tolerance: float = 0.5):
        await self.goto(coordinates=Coordinate(self.state.latitude, self.state.longitude, altitude), tolerance=tolerance)

    async def orbit(self, center: Coordinate, radius: float, speed: float = 5.0, clockwise: bool = True, revolutions: float = 1.0, wait: bool = True) -> Optional[CommandHandle]:
        orbit_time = (2 * math.pi * radius * revolutions) / speed
        angular_speed = (360 * revolutions) / orbit_time * (1 if clockwise else -1)
        start_angle = math.degrees(math.atan2(center.vector_to(self.position).east, center.vector_to(self.position).north))
        ...
        async def execute():
            while orbit_active and time.time() - start_time < orbit_time and not self._aborted:
                elapsed = time.time() - start_time
                tangent = math.radians(start_angle + angular_speed * elapsed + (90 if clockwise else -90))
                await self.set_velocity(VectorNED(speed * math.cos(tangent), speed * math.sin(tangent), 0), heading=self.position.bearing_to(center))
    ...
    def add_waypoint(self, waypoint: Union[Coordinate, Waypoint]):
        self._waypoints.append(Waypoint(coordinate=waypoint) if isinstance(waypoint, Coordinate) else waypoint)
    ...
    async def execute_waypoints(self, tolerance: float = 2.0):
        for wp in self._waypoints:
            await self.goto(coordinates=wp.coordinate, tolerance=wp.acceptance_radius or tolerance, speed=wp.speed)
            if wp.hold_time > 0:
...
class Rover(Vehicle):
    ...
    async def goto(self, latitude: Optional[float] = None, longitude: Optional[float] = None, coordinates: Optional[Coordinate] = None, tolerance: float = 2.0, speed: Optional[float] = None):
        target = Coordinate(coordinates.latitude if coordinates else latitude, coordinates.longitude if coordinates else longitude, 0)
        if speed:
            await self._system.action.set_maximum_speed(speed)
        try:
            await self._system.action.goto_location(target.latitude, target.longitude, 0, 0)
            await self._await_condition(lambda: self.position.ground_distance_to(target) <= tolerance, timeout=300, message="Goto timeout")
        except ActionError as e:
