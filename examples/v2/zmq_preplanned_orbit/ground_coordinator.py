...
class GroundCoordinatorRunner(ZmqStateMachine):
    ...
    def initialize_args(self, extra_args: list[str]) -> None:
        ...
        self._waypoints = read_from_plan_complete(Path(args.file))
    ...
    @expose_zmq("callback_tracer_ready")
    ...
    @expose_zmq("callback_orbiter_ready")
    ...
    @expose_field_zmq("tracer_next_waypoint")
    async def get_tracer_next_waypoint(self, _):
        waypoint = self._waypoints[self._current_waypoint]
        return Coordinate(*waypoint["pos"])

    @expose_field_zmq("orbiter_next_waypoint")
    ...
    @expose_zmq("callback_orbiter_orbit_done")
